...
def generate_pdf(data, risk, status):
    ...
    pdf.multi_cell(0, 10, txt="Disclaimer: This report is generated by an AI research model and is intended for clinical decision support. It is not a replacement for professional medical diagnosis.")
...
res = np.array(proba_out).flatten()
base_proba = float(res) if len(res) > 1 else float(res)
...
t_val = float(np.array(threshold).flatten())
...
with col_left:
    ...
    if status == "HIGH RISK":
        st.markdown(f'<div class="result-card high-risk"><h2>⚠️ {status}</h2><p>High oncogenic risk detected. Immediate clinical referral advised.</p></div>', unsafe_allow_html=True)
    else:
...
st.markdown("---")
st.warning("⚠️ **Disclaimer:** This tool is for research and clinical decision support purposes only. It is not a substitute for professional medical advice, diagnosis, or treatment. Always seek the advice of a qualified health provider with any questions regarding a medical condition.")
