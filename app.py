...
# CSS Fix for Visibility and Contrast
st.markdown('''
    <style>
    .stApp { background-color: #f8f9fa; }
    [data-testid="stSidebar"] { background-color: #1e272e; color: white; }
    /* Fix for invisible titles */
    h1, h2, h3, h4, h5, p, span { color: #2c3e50 !important; }
    ...
    .result-card { padding: 25px; border-radius: 15px; margin-bottom: 20px; }
...
with col_left:
    ...
    if status == "HIGH RISK":
        st.markdown(f'<div class="result-card high-risk"><h2>⚠️ {status}</h2><p>Significant oncogenic markers detected. Clinical referral advised.</p></div>', unsafe_allow_html=True)
    else:
...
with col_right:
    ...
    st.info(f"Model Threshold: {float(threshold):.2f}")
